...
public:
    ...
    // My functions
    //------------------------------------------------------------------------------------
    friend bool _RTLENTRY _EXPFUNC operator>(const complex _FAR &, const complex _FAR &);
    friend bool _RTLENTRY _EXPFUNC operator>=(const complex _FAR &, const complex _FAR &);
    friend bool _RTLENTRY _EXPFUNC operator<(const complex _FAR &, const complex _FAR &);
    friend bool _RTLENTRY _EXPFUNC operator<=(const complex _FAR &, const complex _FAR &);
    complex _FAR & _RTLENTRY operator()(const double& re_part, const double& im_part);
    //-------------------------------------------------------------------------------------
    ...
    friend ostream _FAR & _RTLENTRY _EXPFUNC operator<<(ostream _FAR &, const complex _FAR &);
...
//!!! NEW
inline complex _FAR & _RTLENTRY complex::operator*=(const complex _FAR & __z)
{re = re*__z.re - im*__z.im;
 im = im*__z.re + re*__z.im;
 return *this;
}
...
//---------------------------------------------------------------------------------------
//Definitions of non-member friend complex functions
//---------------------------------------------------------------------------------------
...
//Get imaganiry part of complex value
...
//NEW !!! Calculate the square of the magnitude
inline double  _RTLENTRY _EXPFUNC norm(const complex _FAR & __z)
{
 return __z.re*__z.re + __z.im*__z.im;
}

//NEW !!! Calculate the angle in the plane
inline double  _RTLENTRY _EXPFUNC arg(const complex _FAR & __z)
{
 return (__z == complex(0,0)) ? 0 : ::atan2(__z.im, __z.re);
}

//Create a complex object given polar coordinates:
//__mag - amplitude; __angle - phase => Ampl*exp(jPhi)
...
// Overloaded ANSI C math functions
//----------------------------------------------------------------------------------------
//NEW !!! Calculate absolute value of complex
inline double  _RTLENTRY _EXPFUNC abs(const complex _FAR & __z)
{
...
//NEW !!!  Z = z1*z2 = z1.re*z2.re - z1.im*z2.im + I*(z1.re*z2.im + z1.im*z2.re)
inline complex _RTLENTRY operator*(const complex _FAR & __z1, const complex _FAR & __z2)
{
 return complex(__z1.re*__z2.re - __z1.im*__z2.im, __z1.re*__z2.im + __z1.im*__z2.re);
}
...
//NEW !!! Z = r/z1 = (r*z1.re - I(r*z1.im))/(norm(z1)) = r*conj(z1)/norm(z1)
inline complex _RTLENTRY _EXPFUNC operator/(double _r, const complex _FAR& _z1)
{
 return (_r*conj(_z1))/norm(_z1);
}

//NEW !!! Z = z1/z2 = z1*conj(z2)/norm(z2)
inline complex _RTLENTRY _EXPFUNC operator/(const complex _FAR& _z1, const complex _FAR& _z2)
{
 return (_z1*conj(_z2))/norm(_z2);
}
...
// My functions
//------------------------------------------------------------------------------------------
inline bool _RTLENTRY operator>(const complex _FAR & __z1, const complex _FAR & __z2)
{
  return ((__z1.re * __z1.re + __z1.im * __z1.im) >
          (__z2.re * __z2.re + __z2.im * __z2.im)) ? true : false;
}
...
inline complex _FAR & _RTLENTRY complex::operator()(const double& re_part, const double& im_part)
{ re = re_part;
  im = im_part;
  return *this;
}
...
inline ostream _FAR & _RTLENTRY _EXPFUNC operator<<(ostream _FAR & out, const complex _FAR & z)
{
